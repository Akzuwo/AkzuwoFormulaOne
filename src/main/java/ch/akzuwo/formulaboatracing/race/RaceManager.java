...
import ch.akzuwo.formulaboatracing.utils.TimeUtil;
...
public final class RaceManager {
    ...
    public RaceManager(JavaPlugin plugin, TrackManager trackManager, StorageManager storageManager, RacingBoatManager racingBoatManager, MessageManager messages) {
        this.plugin = plugin;
    ...
    public boolean startRace(Player starter) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (seconds > 0) {
                    broadcastTitle(race, String.valueOf(seconds), "", Sound.BLOCK_NOTE_BLOCK_PLING);
                    seconds--;
                    return;
                }
                beginRunning(race);
                broadcastTitle(race, "GO", "", Sound.ENTITY_PLAYER_LEVELUP);
                cancel();
            }
    ...
    private void cleanupRace(Race race) {
        for (RacePlayer racePlayer : race.getPlayers()) {
            ...
            Player player = racePlayer.getPlayer();
            if (player != null && race.getTrack().getLobbyLocation() != null) {
                player.leaveVehicle();
    ...
    private void broadcastTitle(Race race, String title, String subtitle, Sound sound) {
        for (RacePlayer racePlayer : race.getPlayers()) {
            Player player = racePlayer.getPlayer();
            if (player != null) {
